...
class chatVC: UIViewController , UITableViewDataSource , UITableViewDelegate{
    ...
    override func viewDidLoad() {
        super.viewDidLoad()
//        let voice_Call_Image = UIIm1age(named: "audio_Call")
    //        let voice_Call = UIBarButtonItem(image: voice_Call_Image,  style: .plain, target: self, action: #selector(chatVC.btnVoiceCall(_:)))
                ...
                if let snapShot = querySnapshot?.documents {
                    ...
                    for i in snapShot {
                        if let mainDict = i.data() as? [String : AnyObject] {
                            ...
                            // self.fetchMyMessages()
                            self.arrMSG.append(receivedData(message: message ?? "message not found", time: time ?? "time unavailable", picUrl: picUrl ?? "no image found", audioUrl: audioUrl ?? "no audio found", date: date ?? "date not found",senderId: senderId ?? "" , receiverId: receiverId ?? "", seen: seen ?? false, receiverIsLogin: receiverIsLogin ?? false))
...
extension chatVC : AVAudioRecorderDelegate , AVAudioPlayerDelegate {
    ...
    func showMessageSendingAlert(){
        let alert = UIAlertController(title: "Sending Message...........", message: "It may take few seconds depending upon your connection", preferredStyle: .alert)
